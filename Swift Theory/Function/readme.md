...
```
입출력 파라미터는 내부적으로 copy-in, copy-out 메모리 모델을 사용하여 원본을 '전달'한다.<br>
값을 복사해서 함수 바디(내부)로 전달하고, 종료될 때 Argument로 전달한 변수에 복사된다.<br>
따라서, 변수의 주소를 전달하기 위해 inout 키워드를 사용하여 변수의 주소값을 전달하여 변경하는 방법이다.<br><br>
`inout` 파라미터를 사용하면서<br>
1. 상수(let)나 리터럴 전달할 수 없음
2. 파라미터의 기본값 선언을 허용하지 않음
3. 가변 파라미터(여러개의 파라미터)로 선언하는 것은 불가능함
...
스프링의 `@RestController`, `@Entity`, `@NoArgsConstructor` 같은거라 생각하면 편하다. 물론 기능은 완전 다르다. 느낌이 저런 느낌이다.<br><br>
Swift 5.2 부터 사용하지 않는 변수에 대해서는 `@discardableResult`를 입력하면 경고창이 뜨지 않는다. '버릴 수 있는 결과' 라는 뜻으로 사용되는 것 같다.